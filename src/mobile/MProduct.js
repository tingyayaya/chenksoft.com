...
import banner from '@/images/ba_product.jpg'
import FunctionWraper from './FunctionWraper'
import img1 from '@/images/pro_desb.jpg'
import Title from './Title'

import SmartArchive2 from '@/components/SmartArchive2'

import './mproduct.scss'
import { inject, observer } from 'mobx-react'

import { params } from '@/axios/api.js'
...
class MProduct extends Component {
  ...
  render() {
    ...
    let content;
    if(proId==29) {
      content = <ProductDoc29 {...this.props}/>
    }else {
      content = <ProductQt {...this.props}/>
    }
...
class ProductDoc29 extends Component {
  render() {
   return (
    <div className="mproduct-modules">
      <div className="m-bg-banner">
        <div className="text">
          <h5>文档数字化优势</h5>
          <p>安全、准确、完整；实现纸质档案数据化，有效保护原件</p>
        </div> 
        <img src={banner} alt=""/>
      </div>
...
@inject('store')
@observer
class ProductQt extends Component {
  componentDidMount() {
    const proId = this.props.match.params.proId
    this.props.store.HeaderProApi._getHeaderProduct({domain:`${params.domain}/col.jsp?id=${proId}`})
    this.props.store.CenterProApi._getCenterProduct({domain:`${params.domain}/col.jsp?id=${proId}`})
    this.props.store.FooterProApi._getFootContentProduct({domain:`${params.domain}/col.jsp?id=${proId}`})
  }
  componentWillReceiveProps(nextProps) {
    const proId = nextProps.match.params.proId
    this.props.store.HeaderProApi._getHeaderProduct({domain:`${params.domain}/col.jsp?id=${proId}`})
    this.props.store.CenterProApi._getCenterProduct({domain:`${params.domain}/col.jsp?id=${proId}`})
    this.props.store.FooterProApi._getFootContentProduct({domain:`${params.domain}/col.jsp?id=${proId}`})
  }
  render() {
    const proId = this.props.match.params.proId
    const data = this.props.store.HeaderProApi.data
    const grid = this.props.store.CenterProApi.data
    const title = this.props.store.CenterProApi.title
    const DataContent = this.props.store.FooterProApi.data
    console.log(proId)
    return (
    <div className="mproduct-modules">
      <div className="m-bg-banner">
      ...
      <div className="wraper">
        <FunctionWraper pro={true} title={title} data={grid}/>
      </div>
      {
        proId==25&&<SmartArchive2 mobile={true}/>
      }
      <div className="mproduct-content">
        <ul className="layer1">
          {
            DataContent.map((item, index)=> {
              return <li key={index} className="layer1-li">
                <h5 className="title">{item.title}</h5>
                <ul className="layer2">
                {
                    item.item.map((item2, index2) => {
                      return <li className="layer2-li" key={index2}>
                        <img src={item2.img} alt=""/>
                        {
                            item2.text.map((item3, index3) => {
                              return <div key={index3} className="text">
