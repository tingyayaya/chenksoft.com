...
import SmartArchive from './SmartArchive'
...
class ProductDetail extends Component {
  ...
  render() {
    ...
    return (
      <div>
        {proId!=='29'&&<Header {...this.props}/>}
        {proId!=='29'&&<Grid9 {...this.props}/>}
        {proId==='25'&&<SmartArchive />}
        {proId!=='29'&&<SystemIntroduction {...this.props}/>}
        {proId==='29'&&<DocDigitization />}
      </div>
    )
  }
}
...
class Header extends Component {
  ...
  componentWillMount() {
    ...
    this.props.store.HeaderLableProApi._getHeaderLableProduct({domain:`${params.domain}/col.jsp?id=${proId}`})
  }
  ...
  render() {
    const { data } = this.props.store.HeaderProApi
    const { label } = this.props.store.HeaderLableProApi
    const match = this.props.match;
    return (
      <div className="hb">
        <img src={data.thumb} alt="" className="h-gif" />
        <div className="title">
          <h1 className="title-h">{data.title}</h1>
          <h2 className="sub-title">{data.subTitle}</h2>
          <p>{data.desc}</p>
          <ul className="laber">
            {
              label.map((item, index) => {
                return <Link key={index} to={`${match.url}#proitem12${index}`}  onClick={()=>this.scrollToAnchor(`proitem12${index}`)}><li>{item['skt17.skf188']}</li></Link>
              } )
            }
          </ul>
        </div>
...
class Grid9 extends Component {
  ...
  render() {
    const { title, data } = this.props.store.CenterProApi
    return (
...
class SystemIntroduction extends Component {
  ...
  render() {
    const { data } = this.props.store.FooterProApi
    return (
      <div className="system-introduction">
        <ul className="wraper">
          {
            data.map((item, index) => {
              return <li key={index} id={`proitem12${index}`}>
                      <div className="item-title"><i></i> <span>{item.title}</span> </div>
                      {
                        item.item.map((item2, index2) => {
                          return <div className="item-content" key={index2}>
                                  <div>
